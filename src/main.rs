...
async fn run_docker(env: StdGadgetConfiguration) -> Result<(), gadget_sdk::Error> {
    // Create a new config with defaults
    let private_key = env.keystore()?.ecdsa_key()?.into_inner().signer().seed();
